...
def delete_things():
    ...
    while len(filesToDelete) > 0:
        ...
        try:
            object_client.delete_object(Path="%s" % (thisFile))
        except KeyboardInterrupt:
            sys.exit
        except Exception:
            print("W: trouble deleting %s" % thisFile)
    ...
    if(next_token == ""):
        ...
        try:
            while(listResponse['NextToken'] != ""):
                ...
                listResponse = object_client.list_items(Path=path,
                                                        MaxResults=50,
                                                        NextToken=nt)
                for item in listResponse['Items']:
                    if(item['Type'] == "FOLDER"):
...
if __name__ == "__main__":
    ...
    if(container_path.endswith('/')):
        ...
        container_data = \
            container_client.describe_container(ContainerName=container_name)
        containerEndpoint = container_data['Container']['Endpoint']
        object_client = boto3.client('mediastore-data',
                                     endpoint_url=containerEndpoint)
